...
config_file_path = '/content/converImage/config.conf'
...
def render_image(stored_targets, prev_hash):
        ...
        while True:
            ...
            try:
                ...
                for filename in os.listdir(BlockDir):
                    ...
                    if(submit_image_to_sever(data) is not None):
                        pbar.update(1)
                    os.remove(filepath)
...
if __name__ == "__main__":
    ...
    if(gpu_mode):
        ...
        while i <= num_blocks_to_mine:
            ...
            result = render_image(stored_targets, blockchain[-1]['hash'])
            if not running:
                break
            if result is None:
